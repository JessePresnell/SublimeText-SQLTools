...
SQL_REGEX = {
    'root': [
        ...
        (r'CREATE\s+LOCAL\s+TEMPORARY\s+TABLE\s+', tokens.Keyword.DDL),
        (r'ON\s+COMMIT\s+PRESERVE\s+ROWS\s+AS', tokens.Keyword.DDL),
        (r'CREATE\s+LOCAL\s+TEMP\s+TABLE\s+', tokens.Keyword.DDL),
        (r'GRANT\s+SELECT\s+ON\s+', tokens.Keyword),
        (r'CREATE\s+TEMP\s+TABLE\s+', tokens.Keyword.DDL),
        (r'CREATE\s+TABLE\s+', tokens.Keyword.DDL),
        (r'CREATE(\s+OR\s+REPLACE)?\b', tokens.Keyword.DDL),
        (r'DOUBLE\s+PRECISION\b', tokens.Name.Builtin),
        (r'VARCHAR\(([^)]+)\)\b', tokens.Name.Builtin),
        (r'FLOAT\(([^)]+)\)', tokens.Name.Builtin),
